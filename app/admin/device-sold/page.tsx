...
import { getDeviceSoldData, updateDeviceSoldNote } from "@/utils/dataStorage"
...
export default function DeviceSold() {
  ...
  const fetchDeviceSoldData = useCallback((selectedLocation: string) => {
    const fetchedData = getDeviceSoldData(selectedLocation)
  ...
  useEffect(() => {
    ...
    if (!isLoggedIn) {
      ...
      fetchDeviceSoldData(selectedLocation)
    }
  }, [router, fetchDeviceSoldData])

  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === `deviceSoldData_${location}`) {
        fetchDeviceSoldData(location)
      }
    }

    window.addEventListener("storage", handleStorageChange)
    return () => window.removeEventListener("storage", handleStorageChange)
  }, [location, fetchDeviceSoldData])
  ...
  const handleSaveNote = async (id: number) => {
    await updateDeviceSoldNote(location, id, editedNote)
    fetchDeviceSoldData(location)
    setEditingId(null)
  }
          ...
          <div className="flex justify-between items-center py-6">
            <div className="flex items-center">
              <Image
                ...
                height={32}
              />
              <h1 className="ml-4 text-2xl font-bold text-gray-900">Device Sold</h1>
            </div>
            <div className="flex items-center">
