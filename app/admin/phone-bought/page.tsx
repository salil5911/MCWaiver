...
import { getPhoneBoughtData, updatePhoneBoughtNote } from "@/utils/dataStorage"
...
export default function PhoneBought() {
  ...
  const fetchPhoneBoughtData = useCallback((selectedLocation: string) => {
    const fetchedData = getPhoneBoughtData(selectedLocation)
  ...
  useEffect(() => {
    ...
    if (!isLoggedIn) {
      ...
      fetchPhoneBoughtData(selectedLocation)
    }
  }, [router, fetchPhoneBoughtData])

  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === `phoneBoughtData_${location}`) {
        fetchPhoneBoughtData(location)
      }
    }

    window.addEventListener("storage", handleStorageChange)
    return () => window.removeEventListener("storage", handleStorageChange)
  }, [location, fetchPhoneBoughtData])
  ...
  const handleSaveNote = async (id: number) => {
    await updatePhoneBoughtNote(location, id, editedNote)
    fetchPhoneBoughtData(location)
    setEditingId(null)
  }
          ...
          <div className="flex justify-between items-center py-6">
            <div className="flex items-center">
              <Image
                ...
                height={32}
              />
              <h1 className="ml-4 text-2xl font-bold text-gray-900">Phone Bought</h1>
            </div>
            <div className="flex items-center">
